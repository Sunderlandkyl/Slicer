...
# ------------------------------------------------------------------------------
def _getDicomSupportRules(metadata):
    """Extract and normalize dicom_support_rule from extension metadata.

    :param metadata: Extension metadata dictionary (may be a QVariantMap).
    :return: List of rule strings, or empty list if none found.
    """
    if "dicom_support_rule" not in metadata:
        return []
    ruleValue = metadata["dicom_support_rule"]
    if isinstance(ruleValue, (list, tuple)):
        return [str(r) for r in ruleValue if r]
    return [str(ruleValue)] if ruleValue else []


# ------------------------------------------------------------------------------
def _matchDicomSupportRules(dicomSupportRules, modalities, sopClassUIDs):
    """Check if any dicom_support_rule matches the given DICOM attributes.

    Rules are evaluated using the rule_engine module. Each rule is tested against
    all combinations of the provided modalities and SOP Class UIDs.

    :param dicomSupportRules: List of rule strings to evaluate.
    :param modalities: List of DICOM Modality values (plain Python strings).
    :param sopClassUIDs: List of DICOM SOPClassUID values (plain Python strings).
    ...
    for ruleStr in dicomSupportRules:
        try:
            rule = rule_engine.Rule(str(ruleStr))
            for sopClassUID in (sopClassUIDs if sopClassUIDs else [None]):
                for modality in (modalities if modalities else [None]):
                    ...
                    if sopClassUID is not None:
                        context["SOPClassUID"] = str(sopClassUID)
                    if modality is not None:
                        context["Modality"] = str(modality)
                    try:
                        if rule.matches(context):
                            return True
                    except rule_engine.SymbolResolutionError:
                        # Rule references an attribute not in our context
                        pass
        except Exception as e:
            logging.debug(f"Failed to evaluate rule '{ruleStr}': {e}")
    return False


# ------------------------------------------------------------------------------
def suggestExtensionForDICOMData(dicomAttributes, dataDescription="this DICOM data", offerInstall=False):
    """Check if an extension is available to handle DICOM data that is not supported by installed plugins.

    This function uses the extension manager's dicom_support_rule mechanism to find extensions
    that can handle specific DICOM data types.

    :param dicomAttributes: Dictionary of DICOM attributes (e.g., {"modalities": ["PT"], "sopClassUIDs": ["1.2.840.10008.5.1.4.1.1.128"]})
    :param dataDescription: Human-readable description of the data type (used in log messages)
    :param offerInstall: If True, show a dialog offering to install the extension. If False, just return the extension name.
    :return: Name of suggested extension or None if no extension found

    Example usage::

        dicomAttributes = {"sopClassUIDs": ["1.2.840.10008.5.1.4.1.1.66.4"]}
        extensionName = suggestExtensionForDICOMData(dicomAttributes, "DICOM Segmentation objects", offerInstall=True)
    """
    ...
    try:
        ...
        logging.debug("rule_engine module not available, cannot check for extension suggestions")
        return None

    # Normalize attributes to lists of plain Python strings
    modalities = dicomAttributes.get("modalities", dicomAttributes.get("Modality", []))
    if not isinstance(modalities, list):
        modalities = [modalities]
    modalities = [str(m) for m in modalities if m is not None]

    sopClassUIDs = dicomAttributes.get("sopClassUIDs", dicomAttributes.get("SOPClassUID", []))
    if not isinstance(sopClassUIDs, list):
        sopClassUIDs = [sopClassUIDs]
    sopClassUIDs = [str(s) for s in sopClassUIDs if s is not None]

    extensionsManagerModel = slicer.app.extensionsManagerModel()
    installedExtensions = extensionsManagerModel.installedExtensions

    # Check managed extensions (installed or bookmarked)
    for extensionName in extensionsManagerModel.managedExtensions:
        if extensionName in installedExtensions:
            continue

        # MetadataServer = 2 (from qSlicerExtensionsManagerModel::MetadataSource enum)
        metadata = extensionsManagerModel.extensionMetadata(extensionName, 2)
        if not metadata:
            continue

        dicomSupportRules = _getDicomSupportRules(metadata)
        if dicomSupportRules and _matchDicomSupportRules(dicomSupportRules, modalities, sopClassUIDs):
            if offerInstall:
    ...
    # The model only contains managed extensions, so read the server metadata file directly.
    try:
        import json
        ...
        if not serverMetadataFile:
            logging.debug("Could not find server metadata file, falling back to managed extensions only")
            return None

        with open(serverMetadataFile, 'r') as f:
            serverMetadata = json.load(f)

        for extensionName in serverMetadata:
            ...
            dicomSupportRules = _getDicomSupportRules(metadata)
            if dicomSupportRules and _matchDicomSupportRules(dicomSupportRules, modalities, sopClassUIDs):
                logging.debug(f"Found matching extension '{extensionName}' for {dataDescription}")
                if offerInstall:
                    _offerExtensionInstall(extensionName, dataDescription)
                return extensionName
    except Exception as e:
        logging.debug(f"Failed to check server extensions metadata: {e}")

    return None


# ------------------------------------------------------------------------------
def _offerExtensionInstall(extensionName, dataDescription="this data"):
    """Show a dialog offering to install an extension and install it if user agrees.

    :param extensionName: Name of the extension to offer
    :param dataDescription: Description of the data type that needs the extension
    """
    msgBox = qt.QMessageBox()
    msgBox.setWindowTitle(_("Install Extension"))
    msgBox.setIcon(qt.QMessageBox.Question)
    msgBox.setText(_("The <b>{extensionName}</b> extension can handle {dataDescription}.").format(
        extensionName=extensionName, dataDescription=dataDescription))
    msgBox.setInformativeText(_("Would you like to install the {extensionName} extension now?\n\n"
                                "The application will need to restart after installation.").format(
        extensionName=extensionName))

    installButton = msgBox.addButton(_("Install"), qt.QMessageBox.AcceptRole)
    msgBox.addButton(_("Cancel"), qt.QMessageBox.RejectRole)
        ...
        if not extensionsManagerModel.isExtensionInstalled(extensionName):
            ...
            if not success:
                slicer.util.errorDisplay(
                    _("Failed to install {extensionName} extension.").format(extensionName=extensionName))
        else:
