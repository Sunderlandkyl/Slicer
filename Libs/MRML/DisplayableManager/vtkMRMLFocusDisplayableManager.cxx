...
void vtkMRMLFocusDisplayableManager::UpdateActorRASBounds()
{
  ...
  for (vtkProp* prop: this->Internal->OriginalActors)
    {
    double* currentBounds = prop->GetBounds();
    if (currentBounds)
      {
      for (int i = 0; i < 6; i += 2)
        {
...
void vtkMRMLFocusDisplayableManager::UpdateCornerROIPolyData()
{
  ...
  for (int k = 4; k < 6; ++k)
    ...
    for (int j = 2; j < 4; ++j)
    {
      for (int i = 0; i < 2; ++i)
        {
        double point_RAS[3] = { this->Internal->Bounds_RAS[i], this->Internal->Bounds_RAS[j], this->Internal->Bounds_RAS[k] };
  ...
  int* displaySize = renderer->GetRenderWindow()->GetSize();
